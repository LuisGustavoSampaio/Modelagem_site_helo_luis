(function () {
  ...
  function renderReceiptList(listEl, recs, base) {
    if (!recs || recs.length === 0) {
      listEl.innerHTML = '<div class="empty-state"><p class="empty-state-text">Nenhum comprovante registrado.</p></div>';
      return;
    }

    var h = '<div class="diary-feed-list">';
    recs.forEach(function (r) { h += buildCard(r, base); });
    h += '</div>';
    listEl.innerHTML = h;
  }
    ...
    fetch(base + '/api/receipts')
      ...
      .catch(function () {
        ...
        if (cachedReceipts && cachedReceipts.length > 0) {
          renderReceiptList(listEl, cachedReceipts, base);
          return;
        }
        listEl.innerHTML = '<div class="empty-state"><p class="empty-state-text">Nao foi possivel carregar.</p></div>';
      });
  };
})();
